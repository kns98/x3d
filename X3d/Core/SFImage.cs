...
namespace X3d.Core
{
    ...
    public class SFImage : X3DField
    {
        ...
        public SFImage(
            ...
            byte[,] pixels)
        {
            UpdateImage(componentSize, width, height, pixels);
        }
            ...
            byte[,] pixels)
        {
            if (componentSize == SFImageComponentSize.Unknown)
            {
                if (pixels != null)
                    ...
                    throw new ArgumentException(string.Format(
                        "Component type and pixel data are mismatched. Component Format = {0} and Pixel Format = {1}",
            ...
            else
            {
                if (pixels == null)
                    ...
                    // should not be NULL.
                    throw new ArgumentException(string.Format(
                        "Component type and pixel data are mismatched. Component Format = {0} and Pixel Format = NULL",
                        (int)componentSize));
                ...
                if (width * height != pixels.GetLength(1))
                    ...
                    throw new ArgumentException(string.Format(
                        "Image size and pixel data size are mismatched. Component Format = {0} and Pixel Format = {1}",
                        (int)componentSize, pixels.GetLength(0)));
            }
        }
        ...
        public override string ToString()
        {
            ...
            builder.Append(string.Format("{0} {1} {2}", (int)ComponentSize, Width, Height));

            if (ComponentSize != SFImageComponentSize.Unknown && Width * Height > 0)
        ...
        {
            for (var p = 0; p < obj.Width * obj.Height; p++) builder.AppendFormat(" 0x{0:X2}", obj.Pixels[0, p]);
        }
        ...
        {
            for (var p = 0; p < obj.Width * obj.Height; p++)
                builder.AppendFormat(" 0x{0:X2}{1:X2}{2:X2}{3:X2}", obj.Pixels[0, p], obj.Pixels[1, p],
                    obj.Pixels[2, p], obj.Pixels[3, p]);
        }
        ...
        public override void FromString(string str)
        {
            ...
            ComponentSize = (SFImageComponentSize)int.Parse(tokens[0]);
